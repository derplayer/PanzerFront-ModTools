...
namespace PZFModelEditor.Controls
{
    public partial class MT5Control : UserControl
    {
        ...
        private void listBox_StripEntries_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (typeof(MT5StripAttributes).IsAssignableFrom(listBox_StripEntries.SelectedItem.GetType()))
            {
                MT5StripAttributes attrib = (MT5StripAttributes)listBox_StripEntries.SelectedItem;
                Console.WriteLine("IsUVH: {0}, MirrorU: {1}", attrib.IsUVH, attrib.MirrorU);
            }
        }
